 ...
 */
export interface MainsEvalService {
  getEvaluationStatus: (attemptId: string) => Promise<{ data?: any }>;
  getResults: (attemptId: string) => Promise<{ data?: any }>;
}

interface MainsEvaluatingScreenProps {
  /** sessionStorage key holding the attemptId to poll. */
  attemptIdKey: string;
  /** sessionStorage key used to persist the evaluation start timestamp. */
  evalStartKey: string;
  /** Service exposing getEvaluationStatus / getResults for this flow. */
  service: MainsEvalService;
  /** Route to navigate to once results are ready. */
  resultsRoute: string;
  /** Route for the "back to submission" fallback button. */
  backRoute: string;
}
...
export default function MainsEvaluatingScreen({
  attemptIdKey,
  evalStartKey,
  service,
  resultsRoute,
  backRoute,
}: MainsEvaluatingScreenProps) {
  ...
  const checkResultsReady = useCallback(async () => {
    ...
    return false;
  }, [attemptId, navigateToResults, service]);

  // Get attemptId from sessionStorage on mount
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const storedAttemptId = sessionStorage.getItem(attemptIdKey);
  ...
  const pollStatus = useCallback(async () => {
    ...
    try {
      const res = await service.getEvaluationStatus(attemptId);
      const data = res.data;
  ...
  }, [attemptId, checkResultsReady, service]);
      ...
      >
        ...
        {error && (
          <div className="mt-2 mb-1 px-4 py-2 bg-red-50 border border-red-200 rounded-[10px] text-red-700 text-center" style={{ fontSize: '13px' }}>
            ...
            <div className="mt-2">
              <button
                onClick={() => router.push(resultsRoute)}
