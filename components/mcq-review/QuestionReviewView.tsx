...
/* ─────────────────────────────────────────────────────────────
   ...
   Bookmark) and the footer. Callers just load the questions from
   their own data source and pass them in, so the interaction flow
...
interface QuestionReviewViewProps {
  ...
  /** Where the header/footer "Back" controls point. */
  backHref: string;
  backLabel?: string;
  /** Footer "View Smart Next Steps": either navigate (href) or run a handler (modal). */
  nextStepsHref?: string;
  onViewNextSteps?: () => void;
  /** Source labels used when saving flashcards / bookmarks / spaced-repetition items. */
...
export function isReviewFilter(value: unknown): value is ReviewFilter {
  return typeof value === 'string' && VALID_FILTERS.includes(value as ReviewFilter);
}
    ...
    try {
      if (wasMarked) {
        ...
        const res = await spacedRepService.addItem({
          ...
          answer: correctOpt?.text || q.correctOption,
          subject: q.category,
          source: spacedRepSource,
          sourceType: spacedRepSourceType,
        });
      ...
      <main className="flex-1 flex flex-col items-center px-[clamp(1rem,2vw,3rem)] py-[clamp(1rem,2vh,1.5rem)]">
        <div className="w-full" style={{ maxWidth: 'clamp(700px,60vw,1100px)' }}>
            ...
            <div className="flex items-center gap-3 min-w-0">
              <Link href={backHref}>
                <button className="flex items-center justify-center rounded-full bg-[#F3F4F6] hover:bg-[#E5E7EB] transition-colors flex-shrink-0"
                  style={{ width: '40px', height: '40px' }} aria-label={backLabel}>
          ...
          <div className="flex items-center justify-center gap-[clamp(1rem,1.25vw,1.5rem)] pb-[clamp(1.25rem,1.5vw,2rem)]">
            ...
            {onViewNextSteps ? (
              <button type="button" onClick={onViewNextSteps} className="mcq-act mcq-act-next font-arimo">
                <span className="ic" aria-hidden="true">
