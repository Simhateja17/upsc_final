...
const defaultStudyPlannerFeatures = [
...
const defaults = {
  study_planner_title: 'Your Smart Study Planner',
  study_planner_features: defaultStudyPlannerFeatures,
};

const StudyPlanner = () => {
  const { get } = useCmsContent('home', defaults);
  const plannerFeatures = get('study_planner_features', defaultStudyPlannerFeatures);
        ...
        <div className="flex flex-col lg:flex-row gap-[clamp(2rem,4vw,5rem)] items-center justify-center">
          ...
          <div className="w-full lg:w-[45%] max-w-[500px]">
            ...
            <div className="space-y-[clamp(0.875rem,1.25vw,1.25rem)]">
              {plannerFeatures.map((feature: string, index: number) => (
